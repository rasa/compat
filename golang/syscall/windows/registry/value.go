...
func (k Key) GetMUIStringValue(name string) (string, error) {
	...
	if errors.Is(err, syscall.ERROR_FILE_NOT_FOUND) { // Try fallback path
	...
	for errors.Is(err, syscall.ERROR_MORE_DATA) { // Grow buffer if needed
...
loopItems:
	for i := uint32(0); ; i++ {
		...
		for {
			...
			if errors.Is(err, _ERROR_NO_MORE_ITEMS) {
				break loopItems
			}
			return names, err
		}
