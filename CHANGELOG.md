...
- Add `ErrUmaskChanged` error (Unix-like systems only).
-	Add `ErrInvalidUmask` error (Windows only).
...
- Support `Fstat` on FreeBSD.
- Support `Fstat` on AIX/Dragonfly/OpenBSD/NetBSD/Plan 9/Solaris/Wasm. (absolute paths only)
